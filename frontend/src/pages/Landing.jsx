...
const heroImages = [
  { src: heroImage, alt: 'Apartment building in Nairobi, Kenya' },
  { src: 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&w=900&q=80', alt: 'Modern apartment exterior' },
];
...
const capabilities = [
  { icon: CreditCard, title: 'Rent collection', desc: 'Track receipts, partial payments, arrears, and reminders for every unit.' },
  { icon: Droplets, title: 'Utilities', desc: 'Record water and electricity readings, bill tenants, and keep a clear usage trail.' },
  { icon: Wrench, title: 'Repairs', desc: 'Log tenant issues, assign caretakers, and close work with a visible history.' },
  { icon: Shield, title: 'Tenant risk', desc: 'Spot late-payment patterns before they become a monthly surprise.' },
  { icon: KeyRound, title: 'Rent history', desc: 'Give reliable tenants a verified record they can carry to their next home.' },
  { icon: FileText, title: 'Leases and reports', desc: 'Keep lease dates, renewals, and tax-ready records in one place.' },
  { icon: BellRing, title: 'Tenant messages', desc: 'Send rent reminders, receipts, updates, and broadcasts by SMS, WhatsApp, or USSD.' },
  { icon: BarChart3, title: 'Reports', desc: 'Read collection, vacancy, arrears, and repair numbers without rebuilding spreadsheets.' },
];

const channels = [
  { icon: Building2, label: 'Web portal', desc: 'Dashboards for owners, managers, and office teams.' },
  { icon: CreditCard, label: 'M-Pesa Paybill', desc: 'Match payments to tenants and months without message hunting.' },
  { icon: Phone, label: 'USSD', desc: 'Let tenants check balances or pay from a feature phone.' },
  { icon: Smartphone, label: 'SMS', desc: 'Send reminders, receipts, and repair updates.' },
  { icon: Mic, label: 'Voice', desc: 'Support payment confirmations and access for low-data tenants.' },
  { icon: MessageSquare, label: 'WhatsApp', desc: 'Keep tenant support and receipt delivery close to chat.' },
];
          ...
          <div className="reveal" data-reveal>
            <div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
              <div className="relative min-h-[560px] overflow-hidden rounded-2xl border border-kodi-border bg-kodi-card shadow-2xl animate-float">
                <img src={heroImages[0].src} alt={heroImages[0].alt} className="absolute inset-0 h-full w-full object-cover" />
      ...
      <main className="w-full">
          ...
          <div className="mx-auto max-w-7xl px-5 lg:px-8">
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {capabilities.map(({ icon: Icon, title, desc }, i) => (
                <div key={title} className={`saas-card reveal p-5 stagger-${(i % 4) + 1}`} data-reveal>
          ...
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {channels.map(({ icon: Icon, label, desc }, i) => (
                <div key={label} className={`saas-card reveal p-5 text-center stagger-${i + 1}`} data-reveal>
        ...
        <section className="mx-auto max-w-7xl px-5 py-20 lg:px-8 w-full" data-reveal>
          <div className="max-w-3xl">
            ...
            <p className="section-copy mt-4 text-kodi-text-secondary">
              These are the outcomes we hear from landlords and caretakers who stop juggling tools: faster rent matching, fewer disputes, tighter maintenance timelines, and a calmer week before rent day.
            </p>
          </div>
          ...
          <div className="mt-20 rounded-2xl bg-gradient-to-br from-kodi-card to-kodi-elevated p-8 lg:p-16 border border-kodi-border/30 reveal" data-reveal>
            <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {[
                  ['Portfolio ready', 'Use the same workflow for a few units or a growing property list.'],
                  ['Private records', 'Keep financial records and tenant details in a controlled account.'],
                  ['Local payments', 'Work with the rent channels Kenyan tenants already use.'],
                  ['Team handoff', 'Give caretakers and office users clear tasks instead of scattered messages.'],
                ].map(([title, text], i) => (
                  <div key={title} className={`saas-card saas-card--slate border-white/10 bg-white/5 p-6 stagger-${i + 1}`} data-reveal>
          ...
          <div className="mt-20 rounded-2xl border border-kodi-border bg-kodi-card p-8 shadow-xl lg:p-10">
