...
async function main() {
  console.log('🌱 Seeding KODI database...');
  ...
  const caretaker = await prisma.caretaker.upsert({
    ...
    create: {
      ...
      permissions: ['LOG_PAYMENTS', 'MANAGE_TICKETS', 'VIEW_UNITS'],
    },
  });
  ...
  // 3 consecutive on-time payments for Amina
  const now = new Date();
  for (let i = 2; i >= 0; i--) {
    ...
    await prisma.payment.upsert({
      ...
      create: {
        ...
        channel: 'MPESA',
        paymentDate: new Date(year, adjustedMonth - 1, 1),
        periodMonth: adjustedMonth,
        periodYear: year,
        isPartial: false,
        daysLate: 0,
      },
  ...
  // Trust score and credit passport for Amina
  await prisma.trustScore.upsert({
    where: { tenantId: amina.id },
    update: { score: 640 },
    create: { tenantId: amina.id, score: 640 },
  });
  await prisma.creditPassport.upsert({
    where: { tenantId: amina.id },
    update: { totalMonths: 3, onTimeMonths: 3, lateMonths: 0 },
    create: { tenantId: amina.id, totalMonths: 3, onTimeMonths: 3, lateMonths: 0, averageDaysLate: 0 },
  });

  // Airtime reward (3 consecutive months)
  ...
  console.log('✅ Tenant (Demo 2):', john.name);

  const plumbingTicket = await prisma.maintenanceTicket.upsert({
    where: { id: 'ticket-pipe-burst-001' },
    update: {},
    create: {
      id: 'ticket-pipe-burst-001',
      unitId: 'unit-a2',
      tenantId: john.id,
      category: 'PLUMBING',
      description: 'Pipe burst in bathroom — reported via IVR. Water leaking through ceiling.',
      voiceRecordingUrl: 'https://demo.kodiapp.ke/recordings/ticket-pipe-burst-001.mp3',
      status: 'CLOSED',
      assignedTo: 'Peter Otieno (Caretaker)',
      rating: 4,
  ...
  // John has 1 payment (on time) and 1 late payment
  await prisma.payment.upsert({
    ...
    create: {
      ...
      unitId: 'unit-a2',
      amount: 15000,
      mpesaTransactionId: 'TEST-JOHN-MONTH1',
      channel: 'MPESA',
  ...
  await prisma.trustScore.upsert({
    where: { tenantId: john.id },
    update: { score: 440 },
    create: { tenantId: john.id, score: 440 },
  });
  await prisma.creditPassport.upsert({
    where: { tenantId: john.id },
    update: { totalMonths: 1, onTimeMonths: 0, lateMonths: 1 },
    create: { tenantId: john.id, totalMonths: 1, onTimeMonths: 0, lateMonths: 1, averageDaysLate: 8 },
  });
  console.log('✅ John: plumbing ticket (closed+rated) + late payment seeded');
  ...
  // Grace: 6 months on-time (excellent tenant)
  for (let i = 5; i >= 0; i--) {
  ...
  await prisma.trustScore.upsert({
    where: { tenantId: grace.id },
    update: { score: 780 },
    create: { tenantId: grace.id, score: 780 },
  });
  await prisma.creditPassport.upsert({
    where: { tenantId: grace.id },
    update: { totalMonths: 6, onTimeMonths: 6, lateMonths: 0 },
    create: { tenantId: grace.id, totalMonths: 6, onTimeMonths: 6, lateMonths: 0, averageDaysLate: 0 },
  });

  const diaspora = await prisma.tenant.upsert({
  ...
  // Samuel: partial payment this month (arrears)
  ...
  if (!samuelExisting) {
    await prisma.payment.create({
      data: {
        tenantId: diaspora.id,
        unitId: 'unit-c1',
        amount: 10000,
        mpesaTransactionId: currentTxId,
        channel: 'MPESA',
        paymentDate: new Date(),
        periodMonth: now.getMonth() + 1,
        periodYear: now.getFullYear(),
        isPartial: true,
        daysLate: now.getDate() - 1,
      },
    });
  }

  await prisma.trustScore.upsert({
    where: { tenantId: diaspora.id },
    update: { score: 470 },
    create: { tenantId: diaspora.id, score: 470 },
  });
  await prisma.creditPassport.upsert({
    where: { tenantId: diaspora.id },
    update: { totalMonths: 4, onTimeMonths: 2, lateMonths: 2, partialMonths: 1 },
    create: { tenantId: diaspora.id, totalMonths: 4, onTimeMonths: 2, lateMonths: 2, partialMonths: 1, averageDaysLate: 7 },
  });

  console.log('✅ Additional tenants seeded:', grace.name, diaspora.name);

  // ─── DEMO 3: iTax report for Q1 2025 ─────────────────────────────────────
  const existingReport = await prisma.iTaxReport.findFirst({
    where: { landlordId: landlord.id, year: 2025, quarter: 'Q1' },
  });
  if (!existingReport) {
    await prisma.iTaxReport.create({
      data: {
        ...
        quarter: 'Q1',
        totalIncome: 135000,
        csvUrl: '/uploads/reports/demo-itax-q1-2025.csv',
        pdfUrl: '/uploads/reports/demo-itax-q1-2025.pdf',
      },
    });
    console.log('✅ Demo iTax Q1 2025 report seeded');
  }
  ...
  await prisma.broadcastMessage.create({
    data: {
      ...
      message: 'Dear Tenants, rent is due on 1st of every month. Pay via M-Pesa or dial *384*100#. Thank you — Westlands Heights Management.',
      channel: 'SMS',
      sentAt: new Date(),
      recipientCount: 4,
      status: 'SENT',
